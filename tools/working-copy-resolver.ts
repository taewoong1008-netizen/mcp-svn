...
export class WorkingCopyResolver {
  ...
  async discoverAll(forceRefresh = false): Promise<string[]> {
    const all: string[] = [];
  ...
  async discoverUnder(root: string, forceRefresh = false): Promise<string[]> {
  ...
  async findContainingWorkingCopy(targetPath: string): Promise<string | null> {
    const normalized = path.resolve(targetPath);
  ...
  async resolveCwd(targetPath?: string): Promise<{ cwd: string; resolvedPath?: string }> {
    if (targetPath) {
      if (!this.isAllowedPath(targetPath)) {
        throw new Error(
          `Path "${targetPath}" is outside allowed SVN root(s): ${this.config.roots.join(', ')}`
        );
      }
      const containingWc = await this.findContainingWorkingCopy(targetPath);
      if (!containingWc) {
        throw new Error(
          `Path "${targetPath}" does not belong to any discovered SVN working copy. ` +
    ...
    if (all.length === 0) {
      throw new Error(
        `No SVN working copy found under configured root(s): ${this.config.roots.join(', ')}. ` +
