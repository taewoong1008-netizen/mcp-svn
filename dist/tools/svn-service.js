...
export class SvnService {
     ...
     */
    async configForPaths(paths) {
        if (paths.length === 0) {
            return this.configFor();
        }
        const wcs = await Promise.all(paths.map(p => this.resolver.findContainingWorkingCopy(p)));
        const unique = Array.from(new Set(wcs.filter(Boolean)));
        if (unique.length === 0) {
            throw new SvnError(`None of the provided paths belong to a known SVN working copy. ` +
                `Call svn_list_working_copies to see available WCs.`);
        }
        if (unique.length > 1) {
            throw new SvnError(`Paths span multiple working copies: ${unique.join(', ')}. ` +
    ...
    handleSvnError(error, operation, cwd) {
     ...
     */
    async listWorkingCopies(refresh = false) {
        try {
    ...
    async healthCheck() {
        try {
            ...
            const wcResults = await Promise.all(discovered.map(async (wcPath) => {
     ...
     */
    async diagnoseCommands(targetPath) {
        ...
        try {
            try {
                ...
                if (e.suggestion)
                    results.suggestions.push(e.suggestion);
            }
            try {
    ...
    async getInfo(targetPath) {
        ...
        try {
            ...
            this.handleSvnError(error, 'get SVN info', error.cwd);
        }
    }
    async getStatus(targetPath, showAll = false) {
        try {
            const config = await this.configFor(targetPath);
            const args = ['status'];
            if (targetPath) {
    ...
    async getLog(targetPath, limit, revision) {
        try {
            const config = await this.configFor(targetPath);
            const args = ['log'];
            if (limit && limit > 0)
            ...
            try {
                ...
                if ((error.message?.includes('spawn') && error.message?.includes('ENOENT')) || error.code === 127) {
                    throw new SvnError('SVN is not installed or not in PATH. Install Subversion to use this command.');
                }
                if (error.message?.includes('E175002') ||
                    ...
                    throw new SvnError(`Failed to get log. Possible causes:\n` +
    ...
    async getDiff(targetPath, oldRevision, newRevision) {
        try {
            const config = await this.configFor(targetPath);
            const args = ['diff'];
    ...
    async checkout(url, targetPath, options = {}) {
        try {
            ...
            if (targetPath) {
                ...
                if (!this.resolver.isAllowedPath(targetPath)) {
                    throw new SvnError(`Path "${targetPath}" is outside allowed roots: ${this.resolver.getRoots().join(', ')}`);
                }
            ...
            const config = { ...this.config, workingDirectory: cwd };
            const args = ['checkout'];
            if (options.revision)
    ...
    async update(targetPath, options = {}) {
        try {
            const config = await this.configFor(targetPath);
            const args = ['update'];
            if (options.revision)
    ...
    async add(paths, options = {}) {
        try {
            ...
            for (const p of pathArray) {
                if (!validatePath(p))
                    throw new SvnError(`Invalid path: ${p}`);
            }
