...
// ============================================================
server.tool(
  "svn_list_working_copies",
  "설정된 SVN 루트(들) 아래의 모든 working copy 경로를 재귀 탐색하여 반환합니다. " +
...
server.tool(
  "svn_health_check",
  "SVN 설치 상태 + 발견된 모든 working copy의 유효성·접근성 점검",
  {},
  async () => {
    try {
...
server.tool(
  "svn_diagnose",
  "특정 working copy에 대해 SVN 명령(status/log) 진단 실행. path를 생략하면 첫 번째 WC 대상.",
  {
...
server.tool(
  "svn_info",
  "Working copy 또는 특정 경로/URL의 상세 SVN 정보 조회",
  {
...
server.tool(
  "svn_checkout",
  "SVN repository 체크아웃 (새 working copy 생성)",
  {
...
server.tool(
  "svn_update",
  "Working copy를 repository 최신 상태로 업데이트",
  {
